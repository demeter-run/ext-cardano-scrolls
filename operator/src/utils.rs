...
use crate::{get_config, Error, ScrollsPort};
...
pub async fn build_api_key(crd: &ScrollsPort) -> Result<String, Error> {
    let namespace = crd.namespace().unwrap();
    let network = &crd.spec.network;
    let version = &crd.spec.version;

    let name = format!("scrolls-auth-{}", &crd.name_any());
    ...
    let with_bech = bech32::encode(
        &format!("dmtr_scrolls_{version}_{network}_"),
        base64.to_base32(),
