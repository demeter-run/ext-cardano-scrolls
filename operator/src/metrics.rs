...
use crate::{get_config, Error, ScrollsPort, State};
...
impl Default for Metrics {
    fn default() -> Self {
        ...
        let reconcile_failures = IntCounterVec::new(
            opts!(
                "scrolls_operator_crd_reconciliation_errors_total",
        ...
        let metrics_failures = IntCounterVec::new(
            opts!(
                "scrolls_operator_metrics_errors_total",
...
impl Metrics {
    ...
    pub fn reconcile_failure(&self, crd: &ScrollsPort, e: &Error) {
    ...
    pub fn count_dcu_consumed(&self, project: &str, network: &str, dcu: f64) {
        let service = format!("{}-{}", ScrollsPort::kind(&()), network);
        let service_type = format!("{}.{}", ScrollsPort::plural(&()), ScrollsPort::group(&()));
        let tenancy = "proxy";
...
pub fn run_metrics_collector(state: Arc<State>) {
    tokio::spawn(async move {
        ...
        let network_regex = Regex::new(r"scrolls-([\w]+)-.+").unwrap();
        ...
        loop {
            ...
            let query = format!(
                "sum by (consumer, exported_instance) (increase(scrolls_proxy_http_total_request{{status_code!~\"401|429|503\"}}[{start}s] @ {}))",
